...
# --- CSS PARA BARRA LATERAL E STATUS ---
st.markdown("""<style>
    [data-testid="stSidebar"] { background-color: #FFFFFF !important; }
    [data-testid="stSidebar"] * { color: #000000 !important; }
</style>""", unsafe_allow_html=True)

# --- FUNÇÕES DE PDF (PORTRAIT) ---
...
if not st.session_state['logged_in']:
    ...
    if choice == "Time Tracker":
        ...
        # BUSCAR STATUS E QUAL PROJETO ESTÁ ATIVO (Último log sem data de fim, ou pela session)
        ...
        if working_now and 'active_project' in st.session_state:
            active_project_name = st.session_state['active_project']
        ...
        if not projs.empty:
            ...
            if c2.button("■ STOP", use_container_width=True):
                if 'start_time' in st.session_state:
                    ...
                    if 'active_project' in st.session_state: del st.session_state['active_project']
                    st.rerun()
        ...
        with col_b:
            ...
            if not my_p_info.empty:
                ...
                p_id_sel, p_name_sel = sel_proj.split(" - ")[0], sel_proj.split(" - ")[1]
                p_rem_sel = my_p_info[my_p_info['p_number'] == p_id_sel]['remaining'].values[0]
                if st.button("Generate Detailed PDF"):
                    detailed_logs = pd.read_sql("SELECT date, start_time, end_time, duration FROM time_logs WHERE project_id = ? AND user = ? ORDER BY date DESC", conn, params=(p_id_sel, current_user))
                    pdf_path = generate_detailed_project_pdf(p_id_sel, p_name_sel, detailed_logs, p_rem_sel)
                    with open(pdf_path, "rb") as f: st.download_button("Download Detailed PDF", f, file_name=pdf_path)
