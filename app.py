...
def init_db():
    ...
    # Garantir que as colunas de persistência existam
...
def generate_weekly_pdf(df, start_date):
    ...
    width, height = A4
    if os.path.exists("wigi.png"):
        try: pdf.drawImage("wigi.png", (width/2)-2*cm, height-1.5*cm, width=4*cm, preserveAspectRatio=True)
        except: pass
    ...
    for p_id, row in df.iterrows():
        ...
        line = [p_id, p_name[0] if p_name else "N/A"]
    ...
    table.drawOn(pdf, 1.5*cm, height-6*cm-t_h)
    pdf.save()
    return file_path
...
if not st.session_state['logged_in']:
    ...
    # Opção alterada de "Weekly Reports" para "Reports"
    ...
    if choice == "Project Registration":
        ...
        if working_now and active_p_id:
            ...
            st.markdown(f'<div style="background-color:#E3F2FD; padding:15px; border-radius:8px; border-left:6px solid #1E90FF; color:#1E90FF;">🔵 <b>Status:</b> Working on Project: <b>{p_name[0] if p_name else "Unknown"}</b></div>', unsafe_allow_html=True)
